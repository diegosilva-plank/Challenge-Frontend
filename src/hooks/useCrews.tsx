...
export const useCrews = () => {
  ...
  const showModalToggle = (
    ...
    setCrew?: (value: React.SetStateAction<ICrew>) => void,
    crew?: ICrew
  ) => {
    return () => {
  ...
  const showEditCrewModalToggle = (crew: ICrew) =>
    showModalToggle(setShowEditCrewModal, setCrewEditModal, crew)
  const showDeleteCrewModalToggle = (crew: ICrew) =>
    showModalToggle(setShowDeleteCrewModal, setCrewDeleteModal, crew)
  const showSeeCrewmenModalToggle = (crew: ICrew) =>
    showModalToggle(setShowSeeCrewmenModal, setCrewSeeCrewmenModal, crew)
  ...
  const addCrewmanToCrew = (
    ...
    return async () => {
      ...
      showSeeCrewmenModalToggle(
        crews.filter((crew_filter: ICrew) => crew_filter.id == crew.id)[0]
      )()
      showSeeCrewmenModalToggle(
        crews.filter((crew_filter: ICrew) => crew_filter.id == crew.id)[0]
      )()
  ...
  const removeCrewmanFromCrewFactory = (
    crew: ICrew,
    crewman: ICrewman,
    setShowModal: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
